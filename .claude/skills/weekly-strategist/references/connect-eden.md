...
The user hasn't connected the Eden MCP yet. Give them the exact steps for where they're running you, then have them say "run my weekly sweep" again once connected.
...
4. Say "run my weekly sweep" again.
...
Then `/mcp` to sign in to Eden, and re-run the sweep.
...
Tell the user plainly: "The Eden connection expired. Reconnect it and say run my weekly sweep; I'll pick up where we left off."
...
2. **Write the memo from what you already have.** Fewer findings, honestly labeled. Add one line at the top of the strategy section: "Ran out of Eden research credits partway through, so this week's sweep is partial."
...
4. Still save the memo to the Eden note if note writes are working; the settings and memory notes cost nothing extra to keep consistent.

Never fake findings to fill the gap, and never end the run with only an error message. The user should always get the best memo the remaining data supports, plus one clear line about what happened and the link.

## Any other tool failure

Read the structured error. `invalid` means fix your input and don't retry the same call. `unreachable` means retry once. Anything else: note it in the memo's last line ("couldn't check X this week") and keep going. One broken call never cancels the memo.
