...
The user hasn't connected the Eden MCP yet. Give them the exact steps for where they're running you, then have them say "run my idea engine" again once connected.
...
4. Say "run my idea engine" again.
...
Then `/mcp` to sign in to Eden, and re-run the engine.
...
Tell the user plainly: "The Eden connection expired. Reconnect it and say run my idea engine; I'll pick up where we left off."

## The draft close fails with a write or scope error

`eden_schedule_post` drafts (and note writes) need a write-scoped connection. If a write returns `status: "read-only-token"` or a permissions error, don't fight it: save the draft as a note fallback if notes work, or hand the user the draft text directly, and tell them in one line that removing and re-adding the Eden connector with write access enables one-step drafting next time.
...
2. **Deliver the cards you can stand behind from what you already have.** Fewer cards, honestly labeled. Add one line at the top: "Ran out of Eden research credits partway through, so today's set is partial."
...
4. Still save the run note to the Eden board if note writes are working; settings and memory notes cost nothing extra to keep consistent.

Never fake a card to fill the set, and never end the run with only an error message. The user should always get the best ideas the remaining data supports, plus one clear line about what happened and the link.

## Any other tool failure

Read the structured error. `invalid` means fix your input and don't retry the same call. `unreachable` means retry once. Anything else: note it in one line under the cards ("couldn't check X today") and keep going. One broken call never cancels the run.
