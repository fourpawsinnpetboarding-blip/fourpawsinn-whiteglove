...
Manual export from the Google Business Profile dashboard. Expected columns, header row required:

```
reviewer_name,dog_name,rating,review_text,review_date
```

`dog_name` can be blank. The pipeline tracks which reviews it has already used in `content/reviews/used.json` so it does not repeat one until the export runs out, at which point it says so by name rather than repeating silently.
